resource "aws_lambda_function" "lambda_waf_log_parser_function" {
  ...
  depends_on = [
    aws_iam_role.lambda_log_parser_role,
    aws_wafregional_ipset.waf_blacklist_set,
    aws_wafregional_ipset.waf_scans_probes_set,
    aws_wafregional_ipset.waf_http_flood_set,
  ]
  ...
  role          = aws_iam_role.lambda_log_parser_role[0].arn
  ...
  environment {
    variables = {
      ...
      IP_SET_ID_BLACKLIST                            = aws_wafregional_ipset.waf_blacklist_set[0].id
      IP_SET_ID_AUTO_BLOCK                           = aws_wafregional_ipset.waf_scans_probes_set[0].id
      IP_SET_ID_HTTP_FLOOD                           = aws_wafregional_ipset.waf_http_flood_set[0].id
